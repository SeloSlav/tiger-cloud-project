...
if (existsSync('.env.live')) throw new Error('Live credentials already exist; do not rotate automatically.');
const db = await openTiger();
try {
  // Resolve the endpoint before creating the role, without requesting a password.
  const connection = process.env.DATABASE_URL ?? execFileSync(tigerBinary(),
    ['db','connection-string',process.env.TIGER_SERVICE_ID!,'--skip-update-check'],
    {encoding:'utf8',stdio:['ignore','pipe','pipe'],windowsHide:true}).trim();
  const url = new URL(connection);
  const password = Array.from(randomBytes(32), n => n.toString(16).padStart(2,'0')).join('');
  ...
  url.username='frostline_monitor';url.password=password;url.search='';
  writeFileSync('.env.live',`FROSTLINE_DATABASE_URL=${url}\nFROSTLINE_TLS_MODE=verify-full\n`,{mode:0o600,flag:'wx'});
  console.log('Restricted monitor role created. Credentials are in ignored .env.live; choose the TLS mode for your service before deployment.');
} finally { db.close(); }
